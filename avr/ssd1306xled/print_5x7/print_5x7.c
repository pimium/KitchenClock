...
ssd1306_oled* font5x7_init(){
    font5x7 *f = malloc(sizeof(font5x7));
    font_init(&(f->super));
    f->super.put_string = ssd1306_string_font5x7;
    f->super.put_char = ssd1306_char_font5x7;
    return (ssd1306_oled*) f;
}

void ssd1306_char_font5x7(uint8_t pos_x, uint8_t pos_y, char ch) {
    uint8_t c = ch - 0x20;
    uint8_t message_up[5];
    uint8_t message_down[5];

    for (uint8_t i = 0; i < sizeof(message_up); i++) {
        uint8_t buffer = pgm_read_byte(&ssd1306xled_font5x7[c][i]);
...
void ssd1306_string_font5x7(uint8_t posx, uint8_t posy, char *s) {
    while (*s) {
        ...
        ssd1306_char_font5x7(posx, posy, *s++);
        posx += 6;
    }
}